...
- Real IFRA standard data is not redistributable and is not committed; it is imported by the operator. The app ships a demo set. Place the IFRA 51st Amendment overview PDF or Excel at `data/ifra/standards.pdf` (gitignored) and run `pnpm --filter @fc/api data:import-ifra`. The import links catalog rows by CAS or exact name and adds a reference material for every unmatched standard, including prohibitions. Optional CAS/IUPAC fill-nulls: `pnpm --filter @fc/api data:enrich-cas -- --dry-run`. Operator notes: `apps/api/src/database/data/ifra-operator.md`.
