...
**Goal:** prove each module runs on real input _in your environment_. All of these were run during
...
- **Check `deformVsTechnical.channelsTargetingTechnicalNodes`.** On this asset it is 0. A non-zero
  number on _your_ asset means clips drive nodes that are not joints — read it before binding
...
- **Expect:** a → `exit=0` · b → `exit=0` with skin attributes listed as _added (legal)_ ·
...
`pending`, and the tests passed because they asserted the _list_ rather than the _behaviour_.
...
- **Pass:** rigging touched no geometry. Skin attributes appear as _added (legal)_.
...
- **Expect:** not-ok, with `G1 G3 G6 G10` unevaluated. Judge the run on the other eight. **If any of
  those eight _fails_, that is a real finding.**
