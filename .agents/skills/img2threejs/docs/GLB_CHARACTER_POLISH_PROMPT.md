...
**Do not chase zero.** Where a residual is unavoidable, make it _one-sided_ and say so. Chasing
...
Also check which tier the page loaded. `x3` uses a bigger cell than `x2`, so `x3` is the _coarser_
tier and belongs in `surfaceDataLow.ts`. A swapped mapping shows up as a model that looks _worse_
...
| Node inside-out, or exploded                                             | `NORMAL` missing or flipped on that primitive              | the SDF field's sign comes entirely from the normal, so a wrong normal _inverts_ the surface rather than roughening it                                                     |
...
| Right shape, wrong handedness                                            | rotation used where a reflection was needed                | negate the lateral axis only, then flip triangle winding back — `validate_chirality` catches this, `medial_lateral_bias` catches a pair wrong the _same_ way on both sides |
