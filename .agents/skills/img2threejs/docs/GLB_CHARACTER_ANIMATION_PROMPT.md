...
are there to show _what to measure and what a wrong answer looks like_, not to be copied. The sign
...
| **Bind-pose proportions**          | these _are_ 1:1 and must stay so. Joint positions come from the built model's own measured bounds, so a knee sits where the shin mesh ends. Re-measure after rigging: binding must not move a vertex at rest.                                                            |
...
If the GLB _does_ carry skins or animations, stop and ask. Copying them is a different deliverable
...
- **Skinned**, when one mesh spans a joint. A single node holding the torso _and both arms_ cannot
...
Weighting by raw distance to a bone _segment_ reads the body inside out: a torso surface sits
...
against the ribs is millimetres from the chest _through the air_. Every character whose limbs touch
...
belongs to pipelines that _receive_ a finished mesh. A procedural generator writes its own topology
