...
1.5.1 used _"a clip that neither travels nor rises can repeat seamlessly."_ **That rule does not
...
1. **Deform vs technical.** A node is a `Bone` **iff it appears in `skin.joints`**; everything else
   ...
   **Correction, measured 2026-08-25.** 1.5.2 originally said "clips target technical nodes that are
   ...
   space intact, and a rig that _does_ animate technical nodes is entirely legal glTF — but the
...
recomputed from the mesh's `matrixWorld` on _every_ `updateMatrixWorld`. The skinning evaluation is
therefore:
...
This proves the binding path _reaches the node_. A clip can exist, be loaded, hold an action and
...
Two vertices sitting on top of each other in bind pose but bound to _different_ joints **must**
...
would take — so creases get _worse_ by ~16%. This was accepted deliberately: **a hole shows the
...
Three integration traps, all of which produced _zero visible motion_ in a scene that otherwise
rendered correctly.
...
animation UI: mount the panel as a function so it can also run _after_ the controller exists.
...
Method: render on a saturated background the model cannot contain, count background pixels _inside_
