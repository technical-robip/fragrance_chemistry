...
The ordering matters more than the list. Everything measurable on geometry runs _before_ the browser,
...
| `stage5_rig/geodesic_skinning.py`    | Vertex weights from distance measured _through the solid_; partitions rigid roles out of smooth skinning.         |
...
| `stage4_review/interior_difference.py`                                                                                                                  | Appearance difference _inside_ the silhouette, banded by height. Required on every visual pass.                                 |
