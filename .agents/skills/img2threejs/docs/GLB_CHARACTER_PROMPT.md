...
expect the first run not to be the last. Treat every number in it as an example of _what to measure_,
...
1. **Rigging and animation are usually absent from the asset.** A baseline GLB typically reports
   ...
   your GLB _does_ carry skins or animations, say so and ask before proceeding: matching them is a
...
2. **Texture images, the UV atlas and normal maps are deliberately not copied.** SKILL.md's contract
   ...
   emits no textures. The _values_ above are measured and baked into code; the image files are not
...
| -------------------------- | ---------------------------------------------------------------------------------------- |
| 192 spokes on a glove      | area grew to 1.12x baseline while IoU _fell_ 0.896 → 0.867                               |
...
mapping exactly as below — `x3` uses a bigger cell, so `x3` is the _coarser_ tier:
...
the _same_ way on both sides, which any mirror test passes by construction.
...
Run `joint_loops.py` in _this_ pass even though nothing moves yet. It fails on the surface
