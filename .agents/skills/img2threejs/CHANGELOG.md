...
- Acceptance rule: the character build keeps emitting the same Three.js output for the same
  ...
  domain content is written down _before_ any file moves.
...
- `weapon-v1.4` no longer requires a CS2 marker to pass strict validation. It is the weapon _shape_
...
- Add `forge/stage5_rig/geodesic_skinning.py`: weights from distance measured _through the solid_
...
- Add `forge/stage4_review/scalp_exposure.py`, a HARD gate that finds bald patches geometrically
  ...
  _outside_ the skull — a nearest-neighbour test passes the failing build, because those vertices
...
- `validate_chirality` is a hard spec-time gate naming the actual relation between a pair
  ...
  x _and_ z is a 180° rotation, and rotation preserves handedness, so both limbs come out the same
...
- `chirality.medial_lateral_bias` catches what a pair test structurally cannot: a pair that is wrong
  the _same_ way on both sides is still a perfect mirror of itself. Threshold `0.025` is calibrated
...
- Add `forge/stage4_review/interior_difference.py`: appearance difference _inside_ the silhouette,
