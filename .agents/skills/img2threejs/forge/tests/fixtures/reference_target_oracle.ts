...
export function createTradeItHardCaseModel(options: ProceduralModelOptions = {}): THREE.Group {
  ...
  root.userData.reconstructionEvidence = {
    ...
    referenceCamera: {
      ...
      fovDegrees: 35.0,
      aspect: 1.0,
      orientation: { yaw: 28.0, pitch: 12.0, roll: 0.0 },
      positionHint: [1.1, 0.55, 1.8],
      note: '3/4 front-left studio framing, not solved via solve_camera_pose.py (flat print decal route does not require pixel-exact projection); used only as a review-render hint.',
    },
    approximationNotes: [],
  };
  ...
  materialMap['shellMatte'] = createSculptMaterial(
    ...
    {
      id: 'shellMatte',
      name: 'Rotomolded shell (matte composite)',
      type: 'standard',
      ...
      albedo: {
        dominant: '#212327',
        secondary: ['#242629', '#181a1c'],
        samplingNotes:
          'Estimated by eye from a 400x492px studio photo with light falloff toward the silhouette edge; not spectrophotometric. Near-black, slightly warm dark gray, matte -- no crisp specular hotspot on flat panels.',
      },
      colorVariation: {
        palette: ['#212327', '#242629', '#181a1c'],
        pattern: 'uniform',
        amplitude: 0.06,
      ...
      textureProjection: {
        ...
        anisotropy: 8,
        texelDensityIntent:
          'Preserve stable world-scale detail; decal graphics use a dedicated canvas-baked UV island, not tiled repeat.',
      },
      surfaceFrequencyBands: [
        { id: 'macro', frequency: 1.5, amplitude: 0.1, role: 'broad color/value read' },
        { id: 'meso', frequency: 8.0, amplitude: 0.05, role: 'fine matte grain / brushing' },
        {
          id: 'micro',
          frequency: 32.0,
          amplitude: 0.02,
      ...
      roughness: {
        ...
        localResponse: 'slightly lower roughness on rounded edges/ribs than on flat panels',
      },
      metalness: { base: 0.0, variation: 0.03 },
      normal: {
      ...
      bump: { pattern: 'fine-matte-grain', amplitude: 0.01, scale: 4.0 },
      displacement: { pattern: 'none', amplitude: 0.0, scale: 1.0, silhouetteAffects: false },
      ambientOcclusion: {
        cavityStrength: 0.3,
        contactShadowBias: 0.3,
        notes: 'Darken the lid/body seam, latch mounts, hasp loop, and handle pocket.',
      },
      ...
      localOverrides: [
        {
          id: 'tradeitLogoIcon',
          description:
            'Circular blue double-arrow/handshake icon baked into a canvas decal texture, flat print (#2e57e0 fill, white #f2f2f2 linework), no separate geometry/relief.',
          confidence: 0.75,
        },
        {
          id: 'tradeitWordmark',
          description:
            "White 'TRADEIT' wordmark below the icon, same canvas decal, flat print, no relief.",
          confidence: 0.75,
        },
        {
          id: 'diagonalChevron',
          description:
            'Wide diagonal blue (#2e57e0) wedge/chevron baked into the same canvas decal, widening from upper-right toward lower-left across the front-right third of the shell.',
          confidence: 0.7,
        },
        {
          id: 'frontFacetLines',
          description:
            'Faint vertical panel/facet lines on the lower front face — low-medium confidence, could be render/lighting artifact rather than molded relief; represented as a subtle normal-map micro-band, not geometry, so it can be safely wrong.',
          confidence: 0.35,
        },
      ],
      shaderNotes: [
        'Approximate PBR inference from a single low-resolution studio photo, not extracted measurement; confidence flagged per-field in preSpecAssessment.unknownsToResolveBeforeImplementation.',
      ],
    },
    options,
  );
  materialMap['hardwareSatin'] = createSculptMaterial(
    ...
    {
      id: 'hardwareSatin',
      name: 'Latch/rivet/loop hardware (satin metal)',
      type: 'physical',
      shaderModel: 'MeshStandardMaterial / PBR approximation',
      baseColor: '#8c8e92',
      color: '#8c8e92',
      albedo: {
        dominant: '#8c8e92',
        secondary: ['#a2a4a8', '#5a5c60'],
        samplingNotes:
          'Estimated by eye from a 400x492px studio photo with light falloff toward the silhouette edge; not spectrophotometric. Latch wire bail, rivet/screw heads, hasp eyeloop -- brighter, higher-specular than the shell.',
      },
      colorVariation: {
        palette: ['#8c8e92', '#a2a4a8', '#5a5c60'],
        pattern: 'mottled',
        amplitude: 0.06,
        heightCorrelation: 0.1,
      },
      ...
      metalness: { base: 0.78, variation: 0.03 },
      normal: {
        pattern: 'derived-from-independent-height-field',
        strength: 0.3,
        scale: 20.0,
      ...
      bump: { pattern: 'brushed-linear', amplitude: 0.01, scale: 4.0 },
      displacement: { pattern: 'none', amplitude: 0.0, scale: 1.0, silhouetteAffects: false },
      ambientOcclusion: {
        cavityStrength: 0.3,
        contactShadowBias: 0.3,
  ...
  materialMap['brassPlated'] = createSculptMaterial(
    ...
    {
      id: 'brassPlated',
      name: 'Padlock body (plated brass)',
      type: 'physical',
      shaderModel: 'MeshStandardMaterial / PBR approximation',
      baseColor: '#c9a227',
      color: '#c9a227',
      albedo: {
        dominant: '#c9a227',
        secondary: ['#d8b646', '#8f701a'],
        samplingNotes:
          'Estimated by eye from a 400x492px studio photo with light falloff toward the silhouette edge; not spectrophotometric. Warm gold-yellow plated metal, small in frame (~28x40px), color read solid, roughness estimated.',
      },
      colorVariation: {
        palette: ['#c9a227', '#d8b646', '#8f701a'],
        pattern: 'mottled',
        amplitude: 0.06,
        heightCorrelation: 0.1,
      },
      ...
      metalness: { base: 0.75, variation: 0.03 },
      normal: {
        pattern: 'derived-from-independent-height-field',
        strength: 0.3,
        scale: 20.0,
  ...
  materialMap['steelSatin'] = createSculptMaterial(
    ...
    {
      id: 'steelSatin',
      name: 'Padlock shackle + latch bail (satin steel)',
      type: 'physical',
      shaderModel: 'MeshStandardMaterial / PBR approximation',
      baseColor: '#9a9ca2',
      color: '#9a9ca2',
      albedo: {
        dominant: '#9a9ca2',
        secondary: ['#b4b6bc', '#6a6c72'],
        samplingNotes:
          "Estimated by eye from a 400x492px studio photo with light falloff toward the silhouette edge; not spectrophotometric. Cooler gray than the brass body; shared finish class with hardwareSatin but kept distinct for the shackle's slightly higher metalness/lower roughness observed highlight falloff.",
      },
      colorVariation: {
        palette: ['#9a9ca2', '#b4b6bc', '#6a6c72'],
        pattern: 'mottled',
        amplitude: 0.06,
        heightCorrelation: 0.1,
      },
      ...
      metalness: { base: 0.85, variation: 0.03 },
      normal: {
        pattern: 'derived-from-independent-height-field',
        strength: 0.3,
        scale: 20.0,
  ...
  node_root_0.userData.sculptComponent = {
    ...
    topologyRationale:
      'Rigid molded lower shell assembled from a rounded box blockout with a taper deform; not an organic/continuous surface.',
    geometryDescriptor: {
      topologyIntent: 'tapered rounded box, full width at the top seam narrowing toward the base',
      edgeTreatment: { type: 'rounded', bevelRadius: 0.045, segments: 4 },
      deformationStack: [
        {
          type: 'taper',
          axis: 'y',
          factor: 0.86,
          notes:
            'body narrows toward the base (rotomold draft angle); silhouette-affecting, estimated ~4-6deg/side',
        },
      ],
    ...
    parent: null,
    attachment: null,
    dimensions: { width: 1.0, height: 0.46, depth: 0.78, units: 'relative', confidence: 0.75 },
    transform: { position: [0, 0, 0], rotation: [0, 0, 0], scale: [1, 1, 1] },
    material: 'shellMatte',
    materialLayers: ['shellMatte'],
    colorMaterialRecipe: {
      dominantAlbedo: 'rgba(33, 35, 39, 1.0)',
      secondaryAlbedo: 'rgba(24, 26, 28, 1.0)',
      materialClass: 'plastic',
      materialClassConfidence: 0.6,
      evidence: ['closed-front-3q'],
      samplingNotes:
        'Near-black warm dark gray read from mid-tone shell pixels, discounting studio falloff toward pure black at the silhouette edge.',
    },
    actionProfile: {
      animationRole: 'static',
      pivot: { mode: 'explicit', localPosition: [0, 0, 0], axis: [0, 1, 0], confidence: 0.6 },
      transformChannels: {
        translate: true,
        rotate: true,
        scale: false,
      ...
      collider: {
        ...
        scale: [1, 1, 1],
        isTrigger: false,
        notes: 'Coarse proxy; refine after first browser render.',
      },
      constraints: [],
      destruction: {
        breakable: false,
    ...
    localFeatures: [
      {
        id: 'rounded-bevel-edges',
        description: 'Continuous rounded fillet on all 12 body/lid edges, molded in.',
        confidence: 0.8,
        level: 'macro',
        evidence: ['closed-front-3q'],
      },
      {
        id: 'body-taper',
        description: 'Body narrows from lid seam to base.',
        confidence: 0.7,
        level: 'macro',
        evidence: ['closed-front-3q'],
      },
      {
        id: 'front-facet-lines',
        description:
          'Faint vertical panel lines, low-medium confidence -- see material localOverrides.frontFacetLines.',
        confidence: 0.35,
        level: 'micro',
        evidence: ['closed-front-3q'],
      },
    ],
    evidenceRefs: ['closed-front-3q'],
  };
  ...
  node_lid_1.userData.sculptComponent = {
    id: 'lid',
    name: 'Lid',
    level: 'macro',
    role: 'hinge-lid',
    ...
    topologyClass: 'assembled-solid',
    topologyRationale:
      'Rigid molded upper shell, same construction family as the body, hinged as a rigid rotation not a deforming shell.',
    geometryDescriptor: {
      topologyIntent: 'rounded box slab, front-bottom lip step at the seam',
      edgeTreatment: { type: 'rounded', bevelRadius: 0.04, segments: 4 },
      deformationStack: [],
      uvStrategy: 'generated procedural coordinates',
      normalStrategy: 'vertex normals from generated geometry',
    },
    ...
    attachment: {
      parentId: 'root',
      parentSocket: 'lidHingeSocket',
      localStart: [0, 0.23, -0.39],
      localEnd: [0, 0.23, 0.39],
      contactType: 'pivot-hinge',
      embedDepth: 0.01,
      gapTolerance: 0.004,
      evidenceRefs: ['closed-front-3q'],
    },
    dimensions: {
      width: 1.02,
      height: 0.16,
      depth: 0.7956000000000001,
      units: 'relative',
      confidence: 0.7,
    },
    transform: { position: [0, 0.31, 0], rotation: [0, 0, 0], scale: [1, 1, 1] },
    material: 'shellMatte',
    materialLayers: ['shellMatte'],
    colorMaterialRecipe: {
      dominantAlbedo: 'rgba(33, 35, 39, 1.0)',
    ...
    localFeatures: [
      {
        id: 'lid-front-lip-step',
        description: 'Shallow step in the shell where the lid meets the body seam.',
        confidence: 0.6,
        level: 'meso',
        evidence: ['closed-front-3q'],
      },
    ],
    evidenceRefs: ['closed-front-3q', 'open-mechanism'],
  };
  node_lid_1.userData.actionProfile = {
    animationRole: 'hinge',
    pivot: { mode: 'explicit', localPosition: [0, 0.23, -0.39], axis: [1, 0, 0], confidence: 0.6 },
    transformChannels: {
      translate: false,
      rotate: true,
      scale: false,
  ...
  const mesh_lid_1Geometry = endpoint_lid_1
    ? new THREE.CylinderGeometry(
        endpoint_lid_1.endRadius,
        endpoint_lid_1.baseRadius,
        endpoint_lid_1.length,
        32,
        12,
      )
    : new THREE.BoxGeometry(1, 1, 1, 12, 12, 12);
  ...
  mesh_lid_1.userData.sculptComponent = {
    id: 'lid',
    name: 'Lid',
    level: 'macro',
    role: 'hinge-lid',
  ...
  root.userData.lookDevTargets = {
    ...
    materialPass: {
      ...
      referencePbrExtraction: {
        ...
        skipReason:
          "Flat matte-plastic shell + 2-color printed decal, no patterned/photoreal skin -- scalar PBR values estimated by eye per grimoire/build/threejs_texture_reference.md 'solid albedo for flat paint' rule instead of extract_pbr_evidence.py. See spec.risks[0].",
      },
      mustAvoid: [
...
export function createTradeItHardCaseLookDevLights(
  ...
  lights.userData.lightingFromPhoto = [
    {
      role: 'key',
      direction: 'upper-left, ~45deg elevation',
      colorTemp: 'neutral-cool studio',
      intensity: 'moderate',
      evidence: ['closed-front-3q'],
    },
    {
      role: 'fill',
      direction: 'ambient/bounce, low contrast ratio',
      colorTemp: 'neutral',
      intensity: 'low',
      evidence: ['closed-front-3q'],
    },
    {
      role: 'environment',
      direction: 'soft studio IBL',
      colorTemp: 'neutral',
      intensity: 'low',
      note: 'ACES filmic tone mapping, exposure ~1.0; soft contact shadow under the case on the ground plane.',
      evidence: ['closed-front-3q'],
    },
  ];
