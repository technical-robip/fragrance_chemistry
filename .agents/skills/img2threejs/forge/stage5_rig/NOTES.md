...
result, and that matter to whoever does rig _integration_ later. Read-only
...
| position-only + `BufferGeometryUtils.mergeVertices(geometry, 1e-5)`     | 2050     | 4160              | 0              | 66 _(artifact — see below)_ |
...
(what Milestone 0 uses; see below) is _a_ valid fix, but it was not, as I
originally implied, the _only_ practical one.
...
below. That was an inference from reading the code's _shape_, not something
...
tolerance — I confirmed this by running `mergeVertices` on the _untouched_
