...
window.__IMG2THREEJS_CAPTURE__ = {
  setCamera(cameraSpec) {
    /* apply camera, target, near/far, exposure */
  },
  setReferenceMode({ kind, url }) {
    /* load GLB reference and swap the visible subject */
  },
  capturePass({ passId, mode }) {
    /* select pass and resolve when pixels are ready */
  },
};
```
