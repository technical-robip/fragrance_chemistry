...
- `check_pair()` — enforced at spec time by `validate_sculpt_spec.py`. A pair built by negating x
  _and_ z is a 180° rotation, and rotation preserves handedness, so both limbs come out the same
...
- `medial_lateral_bias()` + `compare_bias()` — needs a reference. Catches what a pair test
  structurally cannot: a pair wrong the _same_ way on both sides is still a perfect mirror of
  ...
  is medial put the big toe outboard on _both_ feet — toe-band mass reference 529 medial / 488
...
penetration gate, which is where `--allow` belongs: parts that _should_ share space (an ear root in a
