...
- **Chirality (spec time, HARD)**: `validate_chirality` in `forge/stage2_spec/validate_sculpt_spec.py`
  requires every `-l`/`-r` pair to be a sagittal MIRROR, not a rotated copy. Negating x _and_ z is a
...
- **Assembly gate (structure, not pixels) — every model ships explodable AND clickable**: this is
  ...
  `userData.explodeWithParent` so it rides its shell; let a named group of _anonymous_ meshes be one
  part while a named group of _named_ parts stays a container. Explode and part-picking must share
