...
| Arbitrary named ROIs in the inventory | **already implemented** | `stage1_intake/build_detail_inventory.py` — `component-zones` mode plus `--components` takes named normalized regions. The _default_ (`DEFAULT_COMPONENT_ZONES`) is only upper/middle/lower thirds, which is what makes it feel grid-only |
...
- `PerspectiveCamera.setViewOffset()` — crop a sub-rectangle of the _original_ projection.
...
scaled to 0.72× on the strength of it. Measured correctly at the _original_ radii the same wings
...
> the same pose shows of it. Keep isolation renders for _structural_ questions (is the spar present,
...
- **Bidirectional / directional Chamfer** — position _and_ orientation of a tear edge.
