...
- **Flat paint / single-colour** (bike frame): use a **solid** albedo (dominant colour) + flat normal. A raw photo crop tiles logos/gradients into stripes across long tubes. Sample the reference's _lit mid-tone_ for the paint hue, then deepen/saturate (a photo mid-tone rendered under studio light reads lighter than the sample).
...
**Sample the CORRECT region per material.** Verify the crop is actually on the part you think — a "handle" crop taken from the blade region gave a navy-blue "grey" handle. Crop, _look at the crop_, then extract.
...
- **Cause 1 — metalness steals the hue.** At `metalness ≈ 0.7–1.0` most of the surface colour is the
  _environment reflection_ (specular F0), not the albedo diffuse; a bright/cool white env reflects
...
- **Cause 2 — a blue-leaning purple reads blue when darkened.** A violet with `B > R`
  ...
  For a purple that _survives_ shading, make it **magenta-leaning, `R ≳ B`** (e.g. 175,48,150).
- **Cause 3 (the real one here) — prose contradicted the reference.** A written brief said
  ...
  **violet at the ricasso → SAPPHIRE-BLUE body (dominant) → navy-black tip**. img2threejs is
  reconstruction-_from-the-image_: when a colour brief disagrees with the reference photo, **sample
