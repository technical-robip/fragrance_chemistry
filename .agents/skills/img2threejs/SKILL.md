...
promise governs how the model is _built_ — it says nothing about which file formats it can
subsequently be _exported_ to; an explicitly-selected emission target (`--target <kind>`) is a
...
Full flags: `grimoire/scripts.md`. Never let a script _score_ visuals — that is the agent's job.
